...
import { Play, Pause, StepForward, RotateCcw, Cpu, Monitor, CheckCircle2, List } from 'lucide-react';
...
export default function UniversalTuringMachine({ programRules, initialData }) {
  ...
  // Calcula o offset dos dados na fita física com base no tamanho do programa
  ...
  // Estado da posição da MTU inicializado dinamicamente
  const [utmHeadPos, setUtmHeadPos] = useState(dataOffset);
  ...
  // Reseta a máquina automaticamente se as props (regras ou dados) mudarem
    ...
    <div className="max-w-5xl mx-auto space-y-6 animate-in fade-in duration-500 pt-4">
      <div className="text-center space-y-3 mb-8">
        ...
        <p className="text-slate-400 max-w-2xl mx-auto text-base">
          A Máquina Universal (MTU) consulta sua própria fita para ler as regras do programa ativo.
        </p>
      </div>

      {/* Referência do Programa Ativo (Visualização Compacta) */}
      <div className="bg-[#0f172a] rounded-xl p-5 border border-slate-800 shadow-lg">
        <div className="flex items-center gap-2 mb-3 text-blue-400 font-semibold text-sm uppercase tracking-wider">
          <List size={18} />
          <h3>Programa M Em Execução</h3>
        </div>
        <div className="flex gap-2 overflow-x-auto custom-scrollbar pb-2">
          {programRules.map((r, i) => (
            <div key={i} className="flex-shrink-0 bg-[#020617] border border-slate-800 rounded px-3 py-1.5 flex items-center gap-2 text-xs font-mono text-slate-300">
              <span className="text-slate-400">{r.fromS}</span>
              <span className="text-blue-400">{r.read}</span>
              <span className="text-slate-600">→</span>
              <span className="text-slate-400">{r.toS}</span>
              <span className="text-purple-400">{r.write}</span>
              <span className="text-slate-500">[{r.move}]</span>
            </div>
          ))}
        </div>
      </div>
